...
def accept(request):
    ...
    """
    return response_with_handover(request, request.GET, render_accept)


def render_accept(request, handover):
...
def render_reject_project(request, handover):
    handover_details = HandoverDetails(handover)
    from_user = handover_details.get_from_user()
    to_user = handover_details.get_to_user()
    project = handover_details.get_project()
    ...
    return render(request, 'accept/reject_reason.html', context)
...
def is_complete(handover):
    return handover.state == State.ACCEPTED or handover.state == State.REJECTED
