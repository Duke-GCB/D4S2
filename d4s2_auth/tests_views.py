...
class OAuthViewsTest(TestCase):
    ...
    def test_authorize_callback(self, mock_login, mock_authenticate, mock_get_token_dict, mock_save_token):
    ...
    @patch('d4s2_auth.views.authenticate')
    @patch('d4s2_auth.views.login')
    def test_authorize_fails_bad_authenticate(self, mock_login, mock_authenticate, mock_get_token_dict, mock_save_token):
        token_dict = {'access_token': 'foo-bar'}
        mock_get_token_dict.return_value = token_dict
